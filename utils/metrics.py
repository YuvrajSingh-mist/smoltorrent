...
def init_worker_metrics(rank: int) -> WorkerMetrics:
    ...
    m = WorkerMetrics(
        bytes_recv=Counter(
            "worker_bytes_recv_total", "Bytes received (store_shard)", ["rank"]
        ),
        bytes_sent=Counter(
            "worker_bytes_sent_total", "Bytes sent (send_shard)", ["rank"]
        ),
        store_ops=Counter(
            "worker_store_ops_total", "Completed store_shard ops", ["rank"]
        ),
        send_ops=Counter("worker_send_ops_total", "Completed send_shard ops", ["rank"]),
        store_errors=Counter(
            "worker_store_errors_total", "Failed store_shard ops", ["rank"]
        ),
        store_duration=Histogram(
...
def init_watcher_metrics(port: int = 8001) -> WatcherMetrics:
    m = WatcherMetrics(
        ...
        last_sync=Gauge(
            "watcher_last_sync_timestamp", "Unix timestamp of last completed sync cycle"
        ),
    )
    m.up.set(1)
    start_http_server(port)
