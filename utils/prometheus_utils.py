...
try:
    ...
    api_store_wall = Histogram("smoltorrent_store_wall_seconds", "End-to-end wall-clock time of /store-shard", buckets=_WALL_BUCKETS)
    api_gather_wall = Histogram("smoltorrent_gather_wall_seconds", "End-to-end wall-clock time of /gather-shards", buckets=_WALL_BUCKETS)
    # process_start_time_seconds is not emitted by prometheus_client on macOS
    # (ProcessCollector uses /proc which doesn't exist on Darwin).
    api_process_start = Gauge("process_start_time_seconds", "Unix timestamp when this process started")
...
class WorkerMetrics:
    ...
    send_ops: object
    store_errors: object
    store_duration: object
    send_duration: object


def init_worker_metrics(rank: int) -> "WorkerMetrics | None":
    if not HAS_PROM:
...
def init_watcher_metrics(port: int = 8001) -> "WatcherMetrics | None":
    if not HAS_PROM:
