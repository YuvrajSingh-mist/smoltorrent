...
def _bar_group(
    ...
    b1 = ax.bar(
        ...
        color=STORE_COLOR,
        capsize=5,
        error_kw={"ecolor": ERR_COLOR, "lw": 1.5},
    )
    b2 = ax.bar(
        x + w / 2,
        gather_means,
    ...
    for bar in list(b1) + list(b2):
        ...
        if h > 0:
            ax.text(
                ...
                h + max(store_errs + gather_errs) * 0.05,
                fmt.format(h),
                ha="center",
                va="bottom",
                fontsize=9,
...
fig, ax = plt.subplots(figsize=(7, 5))
store_tp = [
    RESULTS[m]["size_mb"] / RESULTS[m]["store_s"][i] if RESULTS[m]["store_s"] else 0
    for m in models
    for i in range(len(RESULTS[m]["store_s"]))
]
...
ax.axhline(
    ...
    linewidth=1.2,
    alpha=0.7,
    label="100 Mbps theoretical max",
)
ax.legend(fontsize=10)
ax.set_ylim(0, 14)
fig.tight_layout()
